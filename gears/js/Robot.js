function Robot() {
  ...
  this.load = function (scene, robotStart) {
    ...
    return new Promise(function(resolve, reject) {
      ...
      // PySTEM: optional visual model (glTF/GLB) drawn over the physics
      // box. The file is expected in the body frame (cm, origin at the
      // body centre); it is parented to the box so it follows the physics.
      if (options.modelURL) {
        self.loadModel(scene, body, options);
      }
  ...
  // Reset robot
  // Import options.modelURL and attach it to the body box
  this.loadModel = function(scene, body, options) {
    let url = options.modelURL;
    let slash = url.lastIndexOf('/') + 1;
    let root = url.substring(0, slash);
    let file = url.substring(slash);
    BABYLON.SceneLoader.ImportMesh('', root, file, scene, function(meshes) {
      if (meshes.length == 0) {
        return;
      }
      let top = meshes[0];
      ...
      if (typeof options.modelScale == 'number') {
        top.scaling = top.scaling.multiplyByFloats(options.modelScale, options.modelScale, options.modelScale);
      }
      if (options.modelPosition) {
        top.position = new BABYLON.Vector3(options.modelPosition[0], options.modelPosition[1], options.modelPosition[2]);
      }
      for (let m of meshes) {
        m.isPickable = false;
